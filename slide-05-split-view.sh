...
note "fetch the gas-paying key just for this deploy — never stored in the demo env"
run 'DEPLOYER_KEY=$(doppler secrets get DEPLOY_KEY --project canopy --config dev --plain)'

note "deploying a whole transparency service is one command — a throwaway instance here, so we don't disturb the live forest"
...
note "the payoff: on-chain, the contract's bootstrap key IS the key that signed our root grant — split-view protection lives in the contract, not the operator"
