...
./forestrie deploy --bootstrap-alg es256 --bootstrap-es256-generate --bootstrap-es256-pem-out "$BOOTSTRAP_PEM" --owner-address "$OWNER_ADDRESS" --deployer-key "$DEPLOYER_KEY" --rpc-url "$RPC_URL" --out deployment.json
...
curl -sS -o genesis.cbor "$FORESTRIE_BASE_URL/api/forest/$BOOTSTRAP_LOG_ID/genesis"

./forestrie delegate --coordinator-url "$DELEGATION_COORDINATOR_URL" --log-id "$BOOTSTRAP_LOG_ID" --sign-with "$BOOTSTRAP_PEM" --pinned-registrar-key "$PINNED_REGISTRAR_KEY"

./forestrie create-log --base-url "$FORESTRIE_BASE_URL" --owner-log "$BOOTSTRAP_LOG_ID" --new-log "$BOOTSTRAP_LOG_ID" --sign-with "$BOOTSTRAP_PEM" --self-referential --out-b64 root-grant.b64
...
./forestrie sign-statement --key "$BOOTSTRAP_PEM" --payload statement.json --content-type application/json --out statement.cose

REG_OUT=$(./forestrie register --base-url "$FORESTRIE_BASE_URL" --log-id "$BOOTSTRAP_LOG_ID" --statement statement.cose --grant-b64 "$ROOT_GRANT_B64" --out receipt.cbor 2>&1); echo "$REG_OUT"
...
./forestrie create-log --prepare --base-url "$FORESTRIE_BASE_URL" --owner-log "$BOOTSTRAP_LOG_ID" --new-log "$AUTH_LOG_ID" --auth-log --signer-pem "$DAVID_PEM" --sign-with "$BOOTSTRAP_PEM" --parent-grant-b64 "$ROOT_GRANT_B64" --out-b64 auth-grant.b64

./forestrie delegate --coordinator-url "$DELEGATION_COORDINATOR_URL" --log-id "$AUTH_LOG_ID" --sign-with "$DAVID_PEM" --pinned-registrar-key "$PINNED_REGISTRAR_KEY"

./forestrie create-log --base-url "$FORESTRIE_BASE_URL" --owner-log "$BOOTSTRAP_LOG_ID" --new-log "$AUTH_LOG_ID" --auth-log --signer-pem "$DAVID_PEM" --sign-with "$BOOTSTRAP_PEM" --parent-grant-b64 "$ROOT_GRANT_B64" --out-b64 auth-grant.b64

export AUTH_GRANT_B64=$(cat auth-grant.b64)

./forestrie create-log --prepare --base-url "$FORESTRIE_BASE_URL" --owner-log "$AUTH_LOG_ID" --new-log "$DATA_LOG_ID" --bootstrap-log "$BOOTSTRAP_LOG_ID" --data-log --signer-pem "$DAVID_PEM" --sign-with "$DAVID_PEM" --parent-grant-b64 "$AUTH_GRANT_B64" --out-b64 david-data-grant.b64

./forestrie delegate --coordinator-url "$DELEGATION_COORDINATOR_URL" --log-id "$DATA_LOG_ID" --sign-with "$DAVID_PEM" --pinned-registrar-key "$PINNED_REGISTRAR_KEY"

./forestrie create-log --base-url "$FORESTRIE_BASE_URL" --owner-log "$AUTH_LOG_ID" --new-log "$DATA_LOG_ID" --bootstrap-log "$BOOTSTRAP_LOG_ID" --data-log --signer-pem "$DAVID_PEM" --sign-with "$DAVID_PEM" --parent-grant-b64 "$AUTH_GRANT_B64" --out-b64 david-data-grant.b64

export DATA_GRANT_B64=$(cat david-data-grant.b64)

./forestrie register-grant --base-url "$FORESTRIE_BASE_URL" --owner-log "$AUTH_LOG_ID" --data-log "$DATA_LOG_ID" --bootstrap-log "$BOOTSTRAP_LOG_ID" --signer-pem "$ALICE_PEM" --parent-grant-b64 "$DATA_GRANT_B64" --sign-with "$DAVID_PEM" --out-b64 grant-alice.b64
...
ALICE_REG_OUT=$(./forestrie register --base-url "$FORESTRIE_BASE_URL" --log-id "$DATA_LOG_ID" --statement alice-stmt.cose --grant-b64 "$ALICE_GRANT_B64" --out alice-receipt.cbor 2>&1); echo "$ALICE_REG_OUT"
...
./forestrie verify --genesis genesis.cbor --receipt receipt.cbor --payload statement.cose --entry-id "$ENTRY_ID" --univocity "$UNIVOCITY_ADDRESS" --log-id "$BOOTSTRAP_LOG_ID" --rpc-url "$RPC_URL"
